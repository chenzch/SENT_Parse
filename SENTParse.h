...
//
//---------------------------------------------------------------------------------------------------------------------
#if !defined(SENTPARSE_H)
#define SENTPARSE_H (1)
...
typedef void (*SENTParseResetCallback_t)();
typedef void (*SENTParseDataCallback_t)(uint8_t Data);

typedef struct {
    SENTParseStatus_t Status;
    uint32_t LastRollingCount;
    uint32_t TickLength56;
    SENTParseResetCallback_t ResetCallback;
    SENTParseDataCallback_t DataCallback;
} SENTParseData_t, *LPSENTParseData_t;
